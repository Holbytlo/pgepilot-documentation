...
| SmartBox SB1 | Production (monitoring) | Core services + tunnel are active. Runtime content is rsync-aligned with the current local `sb/devva@45a2fc0` on audited key files, and the box now uses its own SmartBox cloud identity (`sbx_sb1_f9bcdf`). Hostname is still `energity1`. One remaining provisioning-quality issue: relay `machine_id` is currently a non-UUID string and should be replaced before future reprovisioning. |
...
| SmartBox sb7 | Provisioned / waiting on-site activation | Runtime content matches the same audited `sb/devva@45a2fc0` files as SB1/SB4 and the box now uses its own SmartBox cloud identity (`sbx_sb7_70862d`). Deye config is still placeholder/customer-prep only: `enabled:false`, placeholder Solarman IP, placeholder `logger_serial`, so on-site finalization is still required. |
| SmartBox TOU | In progress | Spec ready, not yet implemented on real SB |
| sb-manager | Production (historically unstable) | Live runtime is `main@ef3261b`. PM2 restart counter is `168` as of `2026-04-12 22:20 CEST`; current process uptime is short because of same-day deploy/restart, so treat the counter as historical instability signal, not the current uptime as a health metric. |
...
| Worker runtime sync | **DONE** | worker1/2/3 were backed up and reset on 2026-04-10; refreshed to clean `main@a04e0ba` on 2026-04-12, later unified with `pgepilot_service` on `main@a30c78c`, and are now uniformly running `main@a921042` together with `pgepilot_service` (verified 2026-04-12 22:20 CEST). |
...
| SmartBox identity split | **DONE / follow-up** | SB1, SB4, and SB7 no longer share one cloud identity. Each now has its own `login + smartbox_id + collection_point_id + device_id` in `cp_connector_auth` and matching box configs. Remaining issue: UUID format validation for `machine_id` is still missing in `sb-manager`, and two bad IDs are already present in live config/data (`sb1 relay`, `sb7 inverter`). |
...
| SmartBox runtime alignment + identity split | 04-12 | sb1/sb4/sb7 runtime content was rsynced to current `sb/devva` content (audited key file hashes match local `45a2fc0`), `sb-manager` sync bug was fixed in `ef3261b`, and each box now uses its own SmartBox identity bundle in `cp_connector_auth` |
...
| # | Item | Status | Detail |
|---|------|--------|--------|
| H1 | ~~Synchronize worker1/2/3 with git~~ | **DONE** | Backups stored under `/root/runtime-sync-backups/2026-04-10`; workers were later fully resynchronized and are currently uniform with `pgepilot_service` on `main@a921042`. |
...
| H11 | **Watchdog in provisioning** | **DONE / verify on install** | Live `sb-manager@ef3261b` bootstrap template now contains `enable_hardware_watchdog()` and writes `RuntimeWatchdogSec=30`. Keep verifying it on each new install, but it is no longer just a local-only patch. |
...
| H22 | **machine_id UUID validation in provisioning** | HIGH | `sb-manager/src/routes/admin.js` currently accepts `machine_id` as a generic string. Live data already contains two non-UUID machine IDs (`sb1 relay`, `sb7 inverter`) in SB config and `pge_control.cp_machines`. Tighten validation and repair those IDs consistently before the next provisioning wave. |
...
| 3.19 | **machine_id UUID validation** | Add strict UUID validation to `sb-manager` sync/provisioning APIs and repair the currently bad live IDs (`sb1 relay`, `sb7 inverter`). | Small |
